...
TEST_CASE("Insert Multiple Nodes/Search Testing", "[treap]"){
	...
	t = insert(t,60);
	t = insert(t,70);
	REQUIRE(search(t,30) != nullptr);
	REQUIRE(search(t,40) != nullptr);
	REQUIRE(search(t,50) != nullptr);
	REQUIRE(search(t,60) != nullptr);
	REQUIRE(search(t,70) != nullptr);
